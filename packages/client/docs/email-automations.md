...
On `updateEmailAutomation()` (partial update), omitting `tagActionsOnFinish` leaves the existing finish tags unchanged, exactly like every other field on that method. On `setEmailAutomation()` (full replacement), `tagActionsOnFinish` is required like the other fields — see [Full replacement](#full-replacement) below.
